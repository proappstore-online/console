...
export function DirectoryTab({ token }: { token: string | null }) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/services/developers`)
      .then(async (r) => {
        if (r.ok) setDevs(((await r.json()) as { developers: DevProfile[] }).developers)
        else setError('Failed to load developers')
      })
  ...
  if (error) return <p className="py-8 text-center text-sm text-[var(--error)]">{error}</p>

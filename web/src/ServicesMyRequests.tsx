...
export function MyRequestsTab({ token }: { token: string | null }) {
  ...
  const cancel = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { load(); setCancellingId(null) }
      else {
        const err = await res.json().catch(() => ({ error: 'Cancel failed' })) as { error: string }
        setCancelError(err.error)
        setCancellingId(null)
      }
    } catch (e) { setCancelError((e as Error).message); setCancellingId(null) }
  }
  ...
  return (
    <div className="max-w-2xl space-y-3">
      {cancelError && (
        <div className="rounded-lg bg-[var(--error)]/10 text-[var(--error)] px-4 py-2 text-sm">{cancelError}</div>
      )}
      {requests.map((r) => (
        <div key={r.id} className="rounded-xl border border-[var(--line)] bg-[var(--panel)] p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex-shrink-0">
              {r.status === 'open' && (
                <button type="button" onClick={() => cancel(r.id)} disabled={cancellingId === r.id}
